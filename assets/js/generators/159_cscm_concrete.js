...
function alphaBeta(fc) {
  ...
  const alpha1=0.82, theta1=0.0, lamda1=0.24, beta1=0.33565*Math.pow(fc,-0.95383);
  const alpha2=0.76, theta2=0.0, lamda2=0.26, beta2=0.285*Math.pow(fc,-0.94843);
  return { alpha, theta, lamda, beta, alpha1, theta1, lamda1, beta1, alpha2, theta2, lamda2, beta2 };
}

function additional(fc) {
...
export default function generate(params, meta) {
  const mid = Number(params.mid ?? 159);
  const ro  = Number(params.ro  ?? 0.0023);
  const fc  = Number(params.fc  ?? 40);
  const dmax= Number(params.dmax?? 16);

  const { GF } = fracture(fc, dmax);
  const { G, K } = modulus(fc);
  const ab = alphaBeta(fc);
  const add = additional(fc);
  const { R, XD } = rxd(fc);
  ...
  k += fmtInt(mid) + fmtFixed(ro,4) + fmtInt(1) + fmtFixed(0.0,1) + fmtInt(0) + padLeft("1.05",10) + fmtFixed(0.0,1) + fmtInt(0) + "\n";

  k += "$#    pred\n";
  k += fmtFixed(0.0,1) + "\n";

  k += "$#       g         k     alpha     theta     lamda      beta        nh        ch\n";
  k += fmtFixed(G,1) + fmtFixed(K,1) + fmtFixed(ab.alpha,4) + fmtFixed(ab.theta,7) + fmtFixed(ab.lamda,5) + fmtFixed(ab.beta,7) + fmtFixed(0.0,1) + fmtFixed(0.0,1) + "\n";

  k += "$#  alpha1    theta1    lamda1     beta1    alpha2    theta2    lamda2     beta2\n";
  k += fmtFixed(ab.alpha1,2) + fmtFixed(ab.theta1,1) + fmtFixed(ab.lamda1,2) + fmtFixed(ab.beta1,6) + fmtFixed(ab.alpha2,2) + fmtFixed(ab.theta2,1) + fmtFixed(ab.lamda2,2) + fmtFixed(ab.beta2,7) + "\n";

  k += "$#       r        xd         w        d1        d2\n";
  k += fmtFixed(R,5) + fmtFixed(XD,3) + fmtFixed(0.065,3) + fmtFixed(0.000611,6) + fmtFixed(0.000002,6) + "\n";

  k += "$#       b       gfc         d       gft       gfs      pwrc      pwrt      pmod\n";
  k += fmtFixed(B,1) + fmtFixed(GFC,1) + fmtFixed(D,1) + fmtFixed(GFT,2) + fmtFixed(GFS,2) + fmtFixed(PWRC,1) + fmtFixed(PWRT,1) + fmtFixed(PMOD,1) + "\n";

  k += "$#   eta0c        nc     etaot        nt     overc     overt     srate     rep0w\n";
  k += fmtFixed(add.eta0c,7) + fmtFixed(add.nc,2) + fmtFixed(add.eta0t,7) + fmtFixed(add.nt,2) + fmtFixed(add.overc,5) + fmtFixed(add.overt,5) + fmtFixed(add.srate,1) + fmtFixed(add.repow,1) + "\n";
