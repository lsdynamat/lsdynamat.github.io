...
// Winfrith Concrete (LS-DYNA *MAT_WINFRITH_CONCRETE) — MAT_084
// Header style aligned with KCC sample
// Units: mm-ms-g-N-MPa
...
const MODEL_LINE = "$ Model: Winfrith Concrete - *MAT_WINFRITH_CONCRETE (MAT_084)";

// Optional: if you want references here, add them (keep short)
const REF_LINES = [
  "$ Reference: LS-DYNA Keyword User's Manual — MAT_WINFRITH_CONCRETE",
];

// The “MATERIAL CARDS” banner (same style as KCC)
...
export const DEFAULTS = {
  ...
  pr: 0.2,
  fc: 30.0,    // MPa (UCS)
  dmax: 16.0,  // mm (max aggregate)
};

export const FIELDS = [
  { key: "mid",  label: "Material ID (MID)", unit: "-",      default: DEFAULTS.mid,  min: 1, hint: "Example: 99" },
  { key: "ro",   label: "Density (RO)",      unit: "g/mm^3", default: DEFAULTS.ro,   min: 1e-6, hint: "Typical: 0.0022–0.0024" },
...
export function generate(input = {}) {
  ...
  // Title line (match your preferred style)
  const titleLine = `MAT Concrete WINFRITH, fc=${toFixed(fc, 1)}MPa, dmax=${toFixed(dmax, 0)}mm`;

  const lines = [];

  // ---- Header block (KCC style)
  ...
  const filename = `MAT_WINFRITH_fc=${toFixed(fc, 1)}MPa_dmax=${toFixed(dmax, 0)}mm_mid=${mid}.k`;
  return { filename, keyword };
}

// ------------------------
// math (ported from your Python)
// ------------------------
