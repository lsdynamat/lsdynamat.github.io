...
// IMPORTANT: your website uses materials.json "inputs" (not FIELDS).
// These exports are still useful if you later auto-generate materials.json.
export const FIELDS = [
...
export function generate(input = {}) {
  ...
  // Keep naming as you requested: fc = fc, dmax = dmax
  ...
  // ✅ Title requested: "MAT CSCM fc = fc, dmax = dmax"
  // Use the same for filename suggestion.
  const filename = `MAT_CSCM_fc=${toFixed(fc, 1)}MPa_dmax=${toFixed(dmax, 1)}mm_mid=${mid}.k`;

  // Your page supports either string OR {filename, keyword}.
...
function renderKeyword(f, p) {
  // ✅ Title requested
  const title = `MAT_CSCM_fc=${toFixed(f.fc, 1)}MPa_dmax=${toFixed(f.dmax, 1)}mm`;
