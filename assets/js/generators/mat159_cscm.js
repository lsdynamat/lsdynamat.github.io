...
// Header block (order exactly as requested)
const UNITS_LINE = "$ Units: mm-s-tonne-N-MPa"; // keep exactly like your example (even if site uses mm-ms-g)
const MODEL_LINE = "$ Model: Continuous Surface Cap Model (CSCM) - MAT_159";
...
const GEN_LINE   = "$ Generated by MaterialMap http://materialmap.github.io";
const RULER_LINE = "$--------1---------2---------3---------4---------5---------6---------7---------8";
...
// Optional: for your docs (NOT used by the page, page uses materials.json inputs)
export const FIELDS = [
...
export function generate(input = {}) {
  ...
  // Your requested title style line
  const titleLine = `Concrete, f_c = ${toFixed(fc, 1)} MPa, d_agg = ${toFixed(dmax, 0)} mm`;

  // Build keyword
  const lines = [];

  // IMPORTANT: you asked for this header order
  lines.push("*KEYWORD");
